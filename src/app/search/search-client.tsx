...
export function SearchClient({ initialDocuments, user }: SearchClientProps) {
    ...
    const handleClaim = async (docId: string) => {
        ...
        try {
            ...
            // Optimistically update the UI to show claim is pending
            setDocuments(documents.map(doc => doc.id === docId ? {...doc, claims: [...doc.claims, { claimant: user, status: 'pending', claimDate: new Date().toISOString(), _id: 'temp' }] } : doc));
    ...
    return (
        <>
            ...
            <Input
                ...
                disabled={isPending}
            />
            <Select value={filters.documentType} onValueChange={(value) => handleFilterChange("documentType", value)} disabled={isPending}>
            ...
            <Select value={filters.status} onValueChange={(value) => handleFilterChange("status", value)} disabled={isPending}>
                <SelectTrigger className="w-full sm:w-[180px]">
            ...
            <TableBody>
                {documents.map((doc) => {
                    const isClaimedByCurrentUser = user && doc.claims?.some(c => c.claimant._id === user.id);
                    const canClaim = doc.status === 'found' && user && user.id !== doc.reportedBy && !isClaimedByCurrentUser;
                    ...
                    return (
                        <TableRow key={doc.id}>
                            ...
                            <TableCell>
                                {canClaim ? (
                                    <Button size="sm" onClick={() => handleClaim(doc.id)} disabled={claimingId === doc.id}>
                                        {claimingId === doc.id ? "Claiming..." : <><Hand className="mr-2 h-4 w-4" />Claim</>}
                                    </Button>
                                ) : isClaimedByCurrentUser ? (
                                    <Button size="sm" variant="outline" disabled>Claim Pending</Button>
                                ) : (
